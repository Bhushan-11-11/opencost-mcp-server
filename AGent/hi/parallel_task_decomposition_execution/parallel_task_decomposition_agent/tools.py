...
WEBHOOK_TIMEOUT_SECONDS = int(
    os.getenv("PARALLEL_CONNECTOR_TIMEOUT_SECONDS", "10")
)
WEBHOOK_RETRIES = int(os.getenv("PARALLEL_CONNECTOR_RETRIES", "3"))
...
def _post_json(url: str, payload: dict) -> tuple[int, str]:
    ...
    with urllib_request.urlopen(req, timeout=WEBHOOK_TIMEOUT_SECONDS) as resp:
        ...
        return int(resp.status), response_text[:500]
...
async def _deliver_via_webhook(
    *,
    ...
    for attempt in range(1, WEBHOOK_RETRIES + 1):
        try:
            status_code, body = await asyncio.to_thread(_post_json, url, payload)
            logger.info(
                "%s webhook delivered on attempt %s status=%s",
                connector_name,
                attempt,
                status_code,
            )
            return {
                "status": "success",
                "message": f"{connector_name} webhook delivered (status={status_code}).",
                "response_body": body,
            }
        except (urllib_error.URLError, TimeoutError, ValueError) as exc:
            last_error = exc
            logger.warning(
                "%s webhook delivery failed on attempt %s: %s",
                connector_name,
                attempt,
                exc,
            )
    ...
    raise RuntimeError(
        f"{connector_name} webhook delivery failed after {WEBHOOK_RETRIES} attempts: {last_error}"
    )
...
async def publish_email_announcement(email_content: str) -> dict[str, str]:
    ...
    """
    webhook_url = os.getenv("PARALLEL_EMAIL_WEBHOOK_URL", "").strip()
    if webhook_url:
    ...
    logger.info("Publishing Email Announcement via mock fallback")
    return {"status": "success", "message": "Email announcement published (mock)."}
...
async def publish_slack_message(
    ...
    webhook_url = os.getenv("PARALLEL_SLACK_WEBHOOK_URL", "").strip()
    if webhook_url:
        return await _deliver_via_webhook(
            url=webhook_url,
            payload={"slack_content": slack_content, "channels": channels},
    ...
    return {"status": "success", "message": f"Slack message published to {len(channels)} channels (mock)."}
...
async def create_calendar_event(
    ...
    Users can add their own integration logic here or replace this tool with an MCP tool.
    """
    webhook_url = os.getenv("PARALLEL_CALENDAR_WEBHOOK_URL", "").strip()
    if webhook_url:
        return await _deliver_via_webhook(
            url=webhook_url,
            payload={
    ...
    logger.info("Creating Calendar Event via mock fallback")
    logger.info("  Title: %s", title)
    logger.info("  Description: %s", description)
    logger.info("  Start: %s", start_time)
    logger.info("  End: %s", end_time)
    return {"status": "success", "message": "Calendar event created (mock)."}
