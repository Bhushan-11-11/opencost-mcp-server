...
# Optional webhook connectors used by tools.py for production integrations.
if not os.environ.get("PARALLEL_EMAIL_WEBHOOK_URL"):
    logger.info(
        "PARALLEL_EMAIL_WEBHOOK_URL is not set; email tool will use mock fallback."
    )
if not os.environ.get("PARALLEL_SLACK_WEBHOOK_URL"):
    logger.info(
        "PARALLEL_SLACK_WEBHOOK_URL is not set; Slack tool will use mock fallback."
    )
if not os.environ.get("PARALLEL_CALENDAR_WEBHOOK_URL"):
    logger.info(
        "PARALLEL_CALENDAR_WEBHOOK_URL is not set; calendar tool will use mock fallback."
    )
